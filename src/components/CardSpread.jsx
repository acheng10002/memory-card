...
// parent CardSpread component for LeftSideHeader, RightSideScoreboard, and Cards
function CardSpread() {
  ...
  // state that manages clickedPokemonSet array of images already clicked
  ...
  useEffect(() => {
    ...
    // does it only once when the component mounts and not on subsequent re-renders
  }, []);
  ...
  const handleClick = (event) => {
    ...
    /* if the clickedPokemonSet state does not include cardKey 
    i.e. if the card with that cardKey hasn't been clicked before */
    if (!clickedPokemonSet.includes(cardKey)) {
      // create a new array with the new cardKey
      setClickedPokemonSet([...clickedPokemonSet, cardKey]);

      /* functional form of setScore, ensures that prevScore is the most
      up-to-date value of the state (even if multiple updates are
      batched together, I get the latest state and update it correctly) */
      setScore((prevScore) => {
      ...
      the up-to-date value score, which hasn't been changed yet by setScore(0) */
      ...
      <div className="card-spread">
        ...
        {shuffledPokemonSet.map((pokemonName) => (
          <Card
            ...
            it will get passed down as an identifier for each Card 
            cardKey is for game logic, for tracking which cards have already
            been clicked */
            cardKey={pokemonName}
            /* pokemonName prop is for display
            gets passed down to Card and PokemonImage for display */
